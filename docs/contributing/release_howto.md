...
TODO(evrardjp): Please do not hesitate to complete this section! (skarlso is our expert)
...
Here is a view of the code to run:

0. Create a new branch from main: `git checkout main; git pull upstream main; git checkout -b release-chart-x.y.z;`
...
2. push to branch and open pr. Double-check the following:
   1. helm docs were updated (TODO(skarlso): Clarify expectations here)
   2. apiVersion in the snapshots are correct  (TODO(skarlso): Clarify expectations here)
   3. Update all the helm tests with potential added values (TODO(skarlso): Clarify expectations here)
   4. Update the stability docs with the latest minor version if exists (TODO(skarlso): Clarify expectations here)
   5. Update the CRD conformance tests (TODO(skarlso): Clarify expectations here)
