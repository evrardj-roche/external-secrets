...
| Field | Description | Default | Validation |
| --- | --- | --- | --- |
| `vault` _string_ | Vault defines the vault's name or uuid to access. Do NOT add op:// prefix. This will be done automatically. |  |  |
...
| `onepasswordSDK` _[OnePasswordSDKProvider](#onepasswordsdkprovider)_ | OnePasswordSDK configures this store to use 1Password's new Go SDK to sync secrets. |  | Optional: \{\} <br /> |
